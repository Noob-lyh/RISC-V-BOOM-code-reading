...
为了使写端口的调度变得简单，所有流水线化的浮点单元都被填充到具有相同的延迟。
...
还未实现：可以考虑把这个译码器移动到译码阶段或者读寄存器阶段，因为大部分信号都已经创建好了，只是需要被翻译到Rocket的FPU的语言。
...
FPU单元，包裹了RocketChip的FPU单元（因此包裹了hardfloat，所以数据都是65位）。
