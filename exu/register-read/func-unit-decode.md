...
### RegisterReadDecode 类

读寄存器译码。

代码形式和decode.scala中的译码器部分非常类似。

输入发射指令的valid和微指令，通过查译码表，将微指令的相关成员（控制信号）修改后输出。
 ...
 * 参数supportUnits指示有哪些功能单元正在被使用
 */
  class RegisterReadDecode(supportedUnits: SupportedFuncUnits)
