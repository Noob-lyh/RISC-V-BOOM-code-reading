...
对于一条硬件上的指令（如32位），在经过Decode后，它会成为一个MicroOp类数据的一个成员inst，这个类将是这条指令在后面的流水线中出现的形式。类中还有许多其他的成员，如指示其是否为分支信号、相关控制信号等，指令在流水线中运行的过程中，这些成员会被各个阶段利用、修改，以达到方便编程的目的（相比于每个模块都需要从32位的指令中读取信息，译码后再进行处理而言）。
...
可在构造一些Boom端口类时混入（通常是IO端口类），可以附带一个微指令成员uop。
    ...
    with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
    ...
    val uopc             = UInt(UOPC_SZ.W)       
      // micro-op code 微操作（如NOP、LD、STA等）编码，7位
    val inst             = UInt(32.W)
      // 32位指令
    ...
    val iq_type          = UInt(IQT_SZ.W)        
      // which issue unit do we use?
    val fu_code          = UInt(FUConstants.FUC_SZ.W) 
      // which functional unit do we use?
    val ctrl             = new CtrlSignals	
      // 控制信号，见后面的CtrlSignals类

    
  // What is the next state of this uop in the issue window? useful for the compacting queue.
  // 这条微指令在发射窗口中的下一个状态。用于compacting队列。
  val iw_state         = UInt(2.W)
  // Has operand 1 or 2 been waken speculatively by a load?
  // Only integer operands are speculaively woken up, so we can ignore p3.
  // 操作数1或2是否被一个加载指令推测地唤醒？由于只有整数操作数会被推测地唤醒，第三个操作数可以忽略。
  val iw_p1_poisoned   = Bool()
  val iw_p2_poisoned   = Bool()

    
  val is_br            = Bool()                      
    // is this micro-op a (branch) vs a regular PC+4 inst?
  val is_jalr          = Bool()                      
    // is this a jump? (jal or jalr)
  val is_jal           = Bool()                      
    // is this a JAL (doesn't include JR)? used for branch unit
  val is_sfb           = Bool()                      
    // is this a sfb or in the shadow of a sfb

    
  val br_mask          = UInt(maxBrCount.W)  
  ...
  val imm_packed       = UInt(LONGEST_IMM_SZ.W) 
    // densely pack the imm in decode, then translate and sign-extend in execute
  ...
  val exc_cause        = UInt(xLen.W)          
    // TODO compress this down, xlen is insanity
  val bypassable       = Bool()                      
    // can we bypass ALU results? (doesn't include loads, csr, etc...)
  val mem_cmd          = UInt(M_SZ.W)          
    // sync primitives/cache flushes
  ...
  val is_sys_pc2epc    = Bool()                      
    // Is a ECall or Breakpoint -- both set EPC to PC.
  val is_unique        = Bool()                      
    // only allow this instruction in the pipeline, wait for STQ to
     
    
    // drain, clear fetcha fter it (tell ROB to un-ready until empty)
  val flush_on_commit  = Bool()                      
    // some instructions need to flush the pipeline behind them

    
  // Preditation
  def is_sfb_br        = is_br && is_sfb && enableSFBOpt.B 
    // Does this write a predicate
  def is_sfb_shadow    = !is_br && is_sfb && enableSFBOpt.B 
    // Is this predicated
  val ldst_is_rs1      = Bool() 
    // If this is set and we are predicated off, copy rs1 to dst, else copy rs2 to dst
  ...
  val ldst_val         = Bool()              
    // is there a destination? invalid for stores, rd==x0, etc.
  ...
  val fp_val           = Bool()             
    // is a floating-point instruction (F- or D-extension)?
    // If it's non-ld/st it will write back exception bits to the fcsr.
  val fp_single        = Bool()             
    // single-precision floating point instruction (F-extension)
  ...
  val xcpt_ae_if       = Bool()             
    // I$ access exception.
  val xcpt_ma_if       = Bool()             
    // Misaligned fetch (jal/brjumping to misaligned addr).
...
控制信号。其中的成员在uop经过Decoder时会被赋值。

还未实现：重构它，因为当旁路发生在分支解析之前的阶段时，其中的信息应该不再是正确的。
